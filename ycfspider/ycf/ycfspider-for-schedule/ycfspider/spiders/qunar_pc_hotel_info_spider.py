...
class QunarPcHotelInfoSpider(RedisSpider):
        ...
        try:
            ...
            if picture:
                ...
                if len(js_picture) >= 1:
                    for i in range(len(js_picture)):
                        ...
                        js_headerImage_fine = sel.xpath(xpathstr).extract()
                        if js_headerImage_fine:
                            results["picture_url"] = js_headerImage_fine[0]
                            picture_list_url.append(results["picture_url"])
            # //*[@id="smallImages"]/li[1]
            ...
            if picture_url_li:
                for i in range(len(picture_url_li)):
                    ...
                    if len(sel.xpath(xpathli)) > 0:  # /img/@src'
                        smallImages = sel.xpath(xpathli + '/img/@src').extract()
                        if smallImages:
                            picture_list_url.append(smallImages[0])
            ...
            if picture_js_small:
                for i in range(len(picture_js_small)):
                    ...
                    js_headerImage_small = sel.xpath(xpathjs).extract()
                    if js_headerImage_small:
                        picture_list_url.append(js_headerImage_small)
            ...
            for i in range(infolen):
                ...
                infostr = sel.xpath(infoxpath).extract()
                if infostr:
                    if '开业' in infostr[0]:
                        results['opening_time'] = infostr[0]
                    elif '装修' in infostr[0]:
                        results['decorate_time'] = infostr[0]
                    elif '客房' in infostr[0]:
                        results['rooms'] = infostr[0]
                    elif '层' in infostr[0]:
                        results['storeys'] = infostr[0]
                    else:
                        brandstr = brandstr + infostr[0] + '##'
            ...
            yield Request(sur_tran_url, meta={'item': item}, callback=self.parse_sur_tran, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.QUNAR
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_sur_tran(self, response):
        try:
            ...
            logstr = {"id": results["hotel_id"], "timestamp": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                      "data": json_r}
            self.logger_data.info(json.dumps(logstr))
            if 'data' in json_r.keys():
                data = json_r["data"]
                if 'attrs' in data.keys():
                    attrs = data['attrs']
                    if 'gpoint' in attrs.keys():
                        mapposition = attrs["gpoint"]
            ...
            score_url = "http://review.qunar.com/api/h/" + results["city_name_gb"] + "_" + str(
                results["hotel_id"]) + "/v2/detail?__jscallback=jQuery1830501302233134745_1467714492396"
            yield Request(score_url, meta={'item': item}, callback=self.parse_score, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.QUNAR
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_score(self, response):
        try:
            ...
            if 'data' in json_r.keys():
                data = json_r['data']
                if 'countStat' in data.keys():
                    countStat = data["countStat"]
                    commcnt = 0
                    guruCnt = 0
                    if 'commCnt' in countStat.keys():
                        commcnt = countStat['commCnt']
                    if 'guruCnt' in countStat.keys():
                        guruCnt = countStat['guruCnt']
                    results['total_comment_count'] = u'总评论' + str(guruCnt + commcnt)
                    results["comment_desc"] = u'总评论' + str(guruCnt + commcnt) + ' 条，其中专家评论' + str(guruCnt) + '条'
                if 'hotelScore' in data.keys():
                    results['score'] = data['hotelScore']
                    #  yield item
            sur_hotel_url = "http://hotel.qunar.com/render/detailRecommend.jsp?hotelSEQ=" + results[
                "city_name_gb"] + "_" + str(results["hotel_id"]) + "&startTime=" + (
            ...
            yield Request(sur_hotel_url, meta={'item': item}, callback=self.parse_surhotel, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.QUNAR
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_surhotel(self, response):
        try:
            ...
            self.logger_data.info(json.dumps(logstr))
            sur_hotel = []
            if 'data' in json_respone.keys():
                data = json_respone["data"]
                if 'groupHotels' in data.keys():
                    groupHotels = data["groupHotels"]
                    keys = groupHotels.keys()
                    groupHotels = data["groupHotels"]
                    # print keys
                    i = 1
                    for key in keys:
