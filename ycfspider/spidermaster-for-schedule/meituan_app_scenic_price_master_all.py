...
class MeituanAppScenicPriceMasterAll():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/MeituanAppScenicPriceMasterAll')
        self.mongo_connect()
        ...
        while True:
            try:
                if self.conf.get('LOOP') == '0':
                    if not self.r.get('spider_flag_record:MeituanAppScenicPriceSpider:all'):
                        ...
                        for key in count_key:
                            self.r.delete(key)
                if self.r.len('spider:meituan_app_scenic_price_all') == 0 and self.r.len('MeituanAppScenicPriceSpider:requests') == 0\
                    ...
                    for item in hotel_infos:
                        ...
                        write_dic['ticket_id'] = item['scenic_id']
                        write_dic['city_name_cn'] = item['city_name_cn']
                        write_dic['ticket_name'] = item['scenic_name']
                        write_dic['count'] = count
                        self.push2redis('spider:meituan_app_scenic_price_all', write_dic)
                        # if count == 3000:
                        #     break
                        count += 1
                    hotel_infos = None
                    self.client.close()
                    self.logger.info('meituan_app_scenic_price_master_all write_record data:'+str(count))
                    if self.conf.get('LOOP') == '0':
                        break
            except Exception, e:
...
if __name__=='__main__':
    master = MeituanAppScenicPriceMasterAll()
    master.run()