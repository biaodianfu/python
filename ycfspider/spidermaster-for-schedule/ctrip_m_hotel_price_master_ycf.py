...
class CtripMHotelPriceMasterYcf():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/CtripMHotelPriceMasterYcf')
    ...
    def fill_ctrip_data(self):
        while True:
            try:
                if self.r.len('spider:ctrip_m_hotel_price_ycf') == 0 and self.r.len('CtripMHotelPriceSpider:requests') == 0:
                    self.mysql_connect()
                    ...
                    for item in hotel_infos_cur:
                        if(item[1]==1):
                            if (item[2]!=''):
                                ...
                                self.push2redis('spider:ctrip_m_hotel_price_ycf', write_dic)
                                # if count == 3000:
                                #     break
                                count += 1
                    hotel_infos = None
                    ...
                    self.logger.info('ctrip_m_hotel_price_master_ycf write_record data:'+str(count))
...
if __name__=='__main__':
    master = CtripMHotelPriceMasterYcf()
    master.run()