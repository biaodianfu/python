...
class QunarPcHotelInfoMasterAll():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/QunarPcHotelInfoMasterAll')
        self.mongo_connect()
    ...
    def fill_qunar_data(self):
        while True:
            try:
                if self.r.len('spider:qunar_pc_hotel_info_all') == 0 and self.r.len('QunarPcHotelInfoSpider:requests') == 0:
                   # hotel_infos = list(self.collection.find({}))
                    ...
                    for item in self.collection.find():
                        ...
                        if len(l) == 2:
                            ...
                            write_dic["city"]=l[0]
                            write_dic["id"]=l[1]
                            self.push2redis('spider:qunar_pc_hotel_info_all', write_dic)
                            write_dic.clear()
                        if len(l) == 3:
                        ...
                        count += 1
                    self.client.close()
                    self.logger.info('qunar_pc_hotel_info_master_all_record data:'+str(count))
