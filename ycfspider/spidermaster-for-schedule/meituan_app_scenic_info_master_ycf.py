...
class MeituanAppScenicInfoMasterYcf():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/MeituanAppScenicInfoMasterYcf')
        # self.mysql_connect()
    ...
    def fill_meituanScenic_data(self):
        while True:
            try:
                if self.r.len('spider:meituan_app_scenic_info_ycf') == 0 and self.r.len('MeituanAppScenicInfoSpider:requests') == 0:
                    self.mysql_connect()
                    ...
                    for item in scenic_infos_cur:
                        if(item[1]==4):
                            if (item[2]!=''):
                                ...
                                self.push2redis('spider:meituan_app_scenic_info_ycf', write_dic)
                        #         # if count == 3000:
                        #         #     break
                                count += 1
                    # print count
...
if __name__=='__main__':
    master = MeituanAppScenicInfoMasterYcf()
    master.run()