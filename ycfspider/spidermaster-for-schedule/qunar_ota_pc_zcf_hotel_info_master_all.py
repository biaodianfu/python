...
class QunarOtaPcZcfHotelInfoMaster():
    def __init__(self):
        self.collection=''
        self.r = ''
        self.conf = {}
    ...
    def run(self):
        ...
        self.logger = Logger(self.conf['log_path']+'/QunarOtaPcZcfHotelInfoMaster')
    ...
    def fill_quna_hota_usr_pwd_data(self):
        while True:
            try:
                if self.r.len('spider:qunar_ota_zcf_user_pwd_master') == 0 and self.r.len('QunarOtaPcZcfHotelInfoSpider:requests') ==0: #?
                    count = 1
                    url = self.conf['ota_address'] + '/priceratio/common/product/otausermsg'#'http://192.168.11.92:8080/priceratio/common/product/otausermsg'
                    data = {"channelId":149}
                            ...
                            for item in data:
                                #print type(item)
                                 if 'zcf' in item['tuangouSubUrl']:
                                     ...
                                     url = re.findall(r'.+com',item['tuangouSubUrl'])[0]
                                     write_dic['login_url'] = url.lower()
                                     write_dic['user_name'] = item['loginAccount']
                                     write_dic['password'] = item['loginPassword']
                                     if len(write_dic) == 7:
                                        # print write_dic
                                        self.push2redis('spider:qunar_ota_zcf_user_pwd_master',write_dic)
                                     count += 1
                            # if count <= len(data):
                            self.logger.info('qunar_ota_zcf_user_pwd_master_record data:'+str(count))
...
if __name__=='__main__':
    master = QunarOtaPcZcfHotelInfoMaster()
    master.run()
