...
class StartMaster():
    ...
    def fill_qunar_data(self):
        while True:
            try:
                if self.r.len('spider:qunar_pc_hotel_info_ycf') == 0 and self.r.len('QunarPcHotelInfoSpider:requests') == 0:
                    self.mysql_connect()
                    ...
                    for item in hotel_infos_cur:
                        if(item[1]==2):
                            if (item[2]!=''):
                                ...
                                self.push2redis('spider:qunar_pc_hotel_info_ycf', write_dic)
                                count += 1
                    hotel_infos_cur.close()
                    self.collection.close()
                    self.logger.info('qunar_pc_hotel_info_master_ycf write_record data:'+str(count))
