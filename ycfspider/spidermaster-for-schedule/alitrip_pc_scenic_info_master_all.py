...
class AlitripPcScenicInfoMasterAll():
    def __init__(self):
        self.collection = ''
        self.r = ''
        self.conf = {}
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/AlitripPcScenicInfoMasterAll')
        self.mongo_connect()
    ...
    def fill_ctrip_data(self):
        while True:
            try:
                if self.r.len('spider:alitrip_pc_scenic_info_all') == 0 and self.r.len('spider:AlitripPcScenicInfoSpider') == 0:
                    ...
                    for item in self.collection.find():
                        ...
                        self.push2redis('spider:alitrip_pc_scenic_info_all', write_dic)
                        count += 1
                    self.client.close()
                    self.logger.info('alitrip_pc_scenic_info_master_all_record data:'+str(count))
...
if __name__=='__main__':
    master = AlitripPcScenicInfoMasterAll()
    master.run()