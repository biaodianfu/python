...
class ElongPcHotelInfoMasterAll():
    def __init__(self):
        self.collection = ''
        self.r = ''
        self.conf = {}
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/ElongPcHotelInfoMasterAll')
        self.mongo_connect()
    ...
    def fill_ctrip_data(self):
        while True:
            try:
                if self.r.len('spider:elong_pc_hotel_info_all') == 0 and self.r.len('ElongPcHotelInfoSpider:requests') == 0:
                   # hotel_infos = list(self.collection.find({}))
                    ...
                    for item in self.collection.find():
                        ...
                        self.push2redis('spider:elong_pc_hotel_info_all', write_dic)
                        count += 1
                    self.client.close()
                    self.logger.info('elong_pc_hotel_info_master_all_record data:'+str(count))
...
if __name__=='__main__':
    master = ElongPcHotelInfoMasterAll()
    master.run()