...
class QunarPcHotelPriceMasterYcf():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/QunarPcHotelPriceMasterYcf')
    ...
    def fill_qunar_data(self):
        while True:
            try:
                if self.r.len('spider:qunar_pc_hotel_price_ycf') == 0 and self.r.len('QunarPcHotelPriceSpider:requests') == 0:
                    self.mysql_connect()
                    ...
                    for item in hotel_infos_cur:
                        if(item[1]==2):
                            if (item[2]!=''):
                                ...
                                self.push2redis('spider:qunar_pc_hotel_price_ycf', write_dic)
                                # if count == 3000:
                                #     break
                                count += 1
                    hotel_infos = None
                    ...
                    #self.client.close()
                    self.logger.info('qunar_pc_hotel_price_master_ycf write_record data:'+str(count))
...
if __name__=='__main__':
    master = QunarPcHotelPriceMasterYcf()
    master.run()