...
class StartMaster():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/QunarPcHotelInfoMasterYcf')
    ...
    def fill_qunar_data(self):
        while True:
            try:
                if self.r.len('spider:qunar_pc_hotel_comment_ycf') == 0 and self.r.len('QunarPcHotelCommentSpider:requests') == 0:
                    self.mysql_connect()
                    ...
                    for item in hotel_infos_cur:
                        if(item[1]==2):
                            if (item[2]!=''):
                                ...
                                self.push2redis('spider:qunar_pc_hotel_comment_ycf', write_dic)
                                count += 1
                    hotel_infos_cur.close()
                    self.collection.close()
                    self.logger.info('qunar_pc_hotel_comment_master_ycf write_record data:'+str(count))
