...
class CtripPcScenicInfoMasterYcf():
    ...
    def run(self):
        self.load_conf()
        self.logger = Logger(self.conf['log_path']+'/CtripPcScenicInfoMasterYcf')
        # self.mysql_connect()
    ...
    def fill_ctripScenic_data(self):
        while True:
            try:
                ...
                if self.r.len('spider:ctrip_pc_scenic_info_ycf') == 0 and self.r.len('CtripPcScenicInfoSpider:requests') == 0:
                    self.mysql_connect()
                    ...
                    for item in scenic_infos_cur:
                        if(item[1]==1):
                            ...
                            self.push2redis('spider:ctrip_pc_scenic_info_ycf', write_dic)
                            count += 1
                        #         # if count == 3000:
                        #         #     break
                        #         count += 1
                    ...
                    #self.client.close()
                    self.logger.info('ctrip_pc_scenic_info_ycf write_record data:'+str(count))
...
if __name__=='__main__':
    master = CtripPcScenicInfoMasterYcf()
    master.run()