...
class ElongPcHotelInfoSpider(RedisSpider):
    ...
    def parse(self, response):
        try:
            ...
            results = {"id": "", "desc": "", "retail_price": "", "hotel_id": response.meta['hotelid'],
                       "hotel_name": response.meta['hotelname'], "hotel_used_name": "",
                       "address": response.meta['hoteladdress'], "short_address": "", "channel_id": "3",
                       "hotel_belongs": '0', "country_code": "CN", "country_name": 'china', "province_code": "",
                       "province_name": "", "city_code": "", "city_name_cn": "",
                       "city_name_gb": response.meta['cityNameEn'],
                       "district_code": "", "district_name": "", "postal_code": "", "business": "",
                       "position_type": u"经纬度", "longitude": response.meta['lng'], "latitude": response.meta['lat'],
                       "telephone": "",
                       "email": "", "fax": "",
                       "url": 'http://hotel.elong.com/' + response.meta['cityNameEn'] + '/' + response.meta[
                           'hotelid'] + '/', "picture_url": "", "picture_list_url": "", "comment_desc": "", "brand": "",
                       "type": "", "level": "", "score": "", "total_comment_count": "",
                       ...
                       "trip_advisor_assess": "","avg_position_score":"","avg_facility_score":"","avg_service_score":"","avg_health_score":"",
                        "cost_effective_score":"","avg_comfort_score":"","avg_food_score":"","avg_safety_score":"","avg_scale_score":"",
                       "guest_impression":""}

            hotelStr = response.body
            hotellogstr = {"id": results["hotel_id"],
                           "timestamp": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "data": hotelStr}
            self.logger_data.info(hotellogstr)
            ...
            parse_wifi_dict = {"1": "无线WIFI", "2": "停车场", "3": "接机服务", "4": "餐厅", "5": "商务中心", "6": "会议服务", "7": "游泳池",
                               "8": "健身房", "9": "宽带"}
            ...
            if comment_msg1:
                ...
                # results['good_comment_count'] = gassess
                # results['bad_comment_count'] = bassess
            # print results['gassess']
            # print results['bassess']
            # print results.get("total_comment_count")
            # print "好评率：" + results["good_comment_rate"]
            ...
            hoteldoc = soup.find(attrs={"id": "hotelContent"}).findAll(
                attrs={"class": "dview_info_item"})  # 解析酒店电话、开业时间等
            number = len(hoteldoc)
            ...
            # 收藏数量
            favourite_count_url = "http://hotel.elong.com/ajax/detail/favhotle/favcount/?callback=jQuery1111014820067783092095_1467603429906&hotelId=" + \
                                  response.meta["hotelid"]
            yield Request(favourite_count_url,
                          meta={'item': item, "results": results, 'cityId': response.meta['cityId']},
                          callback=self.parse_favourite_count, dont_filter=True)
        except:
            error_log_dic = {}
    ...
    def parse_favourite_count(self, response):  # 解析收藏数量
        try:
            ...
            if len(value) > 0:
                valuestr = {"id": results["hotel_id"],
                            "timestamp": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "data": value}
                self.logger_data.info(valuestr)
            # print value
            results["favourite_count"] = str(value["value"])
            # 周边景点
            surrAttraUrl = 'http://hotel.elong.com/ajax/detail/getpositionjva/?request.hotelId=' + results[
                "hotel_id"] + '&request.q=' + u'景点' + '&request.lng=' + results["longitude"] + '&request.lat=' + \
                           results["latitude"] + '&request.page_num=' + u'1' + '&request.filter=' + u'1'
            yield Request(surrAttraUrl,
                          meta={'item': response.meta['item'], "results": results, 'cityId': response.meta['cityId']},
                          callback=self.parse_sur_sce, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.ELONG
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_sur_sce(self, response):
        try:
            ...
            surTraUrl = 'http://hotel.elong.com/ajax/detail/gettrafficjva/?cityId=' + response.meta[
                'cityId'] + '&request.hotelId=' + results["hotel_id"] + '&request.q=' + u'长途汽车站' + '&request.lng=' + \
                        results["longitude"] + '&request.lat=' + results["latitude"] + '&request.page_num=' + u'0'
            yield Request(surTraUrl,
                          meta={'item': response.meta['item'], "results": results, 'cityId': response.meta['cityId']},
                          callback=self.parse_sur_traffic, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.ELONG
            error_log_dic['platform_id'] = PlatformEnum.PC
        ...
        try:
            ...
            try:
                ...
                logstr = {"id": results["hotel_id"], "timestamp": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                          "data": surTraJson}
                self.logger_data.info(logstr)
                surTraValue = surTraJson.get('value')
                if surTraValue :
                    surTraffic = []
                    ...
                    for j in range(surTraValuelen):
                        ...
                        for k in range(surTraValue[j]["total"]):
                            typeStr = typeStr + str(k + 1) + "##" + surTraValue[j]["trafficDetail"][k][
                                "stationName"] + "##" + u"直线距离" + str(
                                surTraValue[j]["trafficDetail"][k]["distance"]) + u"m;"
                        dict = {type: typeStr}
                ...
                surhotUrl = 'http://hotel.elong.com/ajax/detail/getsurroundinghotels/?cityId=' + response.meta[
                    'cityId'] + '&cityNameEn=' + results["city_name_gb"] + '&lat=' + results["latitude"] + '&lng=' + \
                            results["longitude"] + '&hotelId=' + str(results["hotel_id"]) + '&starLevel=' + str(
                    results["level"])
                # print surhotUrl
                yield Request(surhotUrl, meta={'item': response.meta['item'], "results": results},
                              callback=self.parse_sur_hotel, dont_filter=True)
            except:
                yield Request(surhotUrl, meta={'item': response.meta['item'], "results": results},
                              callback=self.parse_sur_traffic, dont_filter=True)
        except:
        ...
        try:
            ...
            if len(response.body) > 75:
                ...
                for surHotelNumber in range(surHotelDoclen):
                    surHotelTextString = surHotelTextString + str(surHotelNumber + 1) + '##' + str(
                        surHotelDoc[surHotelNumber].find(attrs={"class": "dsider_per_text"}).find(
                            'a').text) + '##' + str(
                        surHotelDoc[surHotelNumber].find(attrs={"class": "dsider_per_text"}).find('p').text) + ';'
                # item["surHotel"]=str(surHotelTextString)
                results["surround_hotel"] = surHotelTextString.decode('utf8').replace("\t", "").replace(" ",
                                                                                                        "").replace(
                    "\n", "")
                # print results
                # log.msg("The hotel Info item:::"+str(item),loglevel=log.INFO)
                # self.elonglogger.info(item)
                sur_food_url = "http://hotel.elong.com/ajax/detail/getpositionjva/?request.hotelId=" + str(
                    results["hotel_id"]) + "&request.q=美食&request.lng=" + results["longitude"] + "&request.lat=" + \
                               results["latitude"] + "&request.page_num=1"
                yield Request(sur_food_url, meta={'item': response.meta['item'], "results": results},
                              callback=self.parse_surround_food, dont_filter=True)
            else:
                pass
        except:
            error_log_dic = {}
            ...
            if sur_food_total > 0:
                ...
                for data in trafficItems:
                    ...
                    if "price" in data.keys():
                        surround_food_list = surround_food_list + str(i) + "##" + data["name"] + "##直线约" + str(
                            data["distance"]) + "米##人均" + str(data["price"]) + "元##" + data["address"] + ";"
                    else:
                        surround_food_list = surround_food_list + str(i) + "##" + data["name"] + "##直线约" + str(
                            data["distance"]) + "米##人均0元##" + data["address"] + ";"
                    i = i + 1
                results["surround_food"] = surround_food_list.replace("\t", "").replace(" ", "").replace("\n", "")
            else:
                results["surround_food"] = ''
            # print results
            surround_entertainment_url = "http://hotel.elong.com/ajax/detail/getpositionjva/?request.hotelId=" + str(
                results["hotel_id"]) + "&request.q=娱乐&request.lng=" + str(results["longitude"]) + "&request.lat=" + str(
                results["latitude"]) + "&request.page_num=1"
            yield Request(surround_entertainment_url, meta={'item': response.meta['item'], "results": results},
                          callback=self.parse_surround_entertainment, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.ELONG
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_surround_entertainment(self, response):
        try:
            ...
            if surround_entertainment_total > 0:
                ...
                for data in trafficItems:
                    if "price" in data.keys():
                        surround_entertainment_list = surround_entertainment_list + str(i) + "##" + data[
                            "name"] + "##直线约" + str(data["distance"]) + "米##人均" + str(data["price"]) + "元##" + str(
                            data["address"]) + ";"
                    else:
                        surround_entertainment_list = surround_entertainment_list + str(i) + "##" + data[
                            "name"] + "##直线约" + str(data["distance"]) + "米##人均0元##" + str(data["address"]) + ";"
                    i = i + 1
                # print type(surround_entertainment_list)
                results["surround_entertainment"] = surround_entertainment_list.replace("\t", "").replace(" ",
                                                                                                          "").replace(
                    "\n", "")
            else:
                results["surround_entertainment"] = ''
            # print results
            surround_shopping_url = "http://hotel.elong.com/ajax/detail/getpositionjva/?request.hotelId=" + str(
                results["hotel_id"]) + "&request.q=购物&request.lng=" + str(results["longitude"]) + "&request.lat=" + str(
                results["latitude"]) + "&request.page_num=1"
            yield Request(surround_shopping_url, meta={'item': response.meta['item'], "results": results},
                          callback=self.parse_surround_shopping, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.ELONG
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_surround_shopping(self, response):
        try:
            ...
            if surround_shopping_total > 0:
                ...
                for data in trafficItems:
                    if "price" in data.keys():
                        surround_shopping_list = surround_shopping_list + str(i) + "##" + data["name"] + "##直线约" + str(
                            data["distance"]) + "米##人均" + str(data["price"]) + "元##" + str(data["address"]) + ";"
                    else:
                        surround_shopping_list = surround_shopping_list + str(i) + "##" + data["name"] + "##直线约" + str(
                            data["distance"]) + "米##人均0元##" + str(data["address"]) + ";"
                    i = i + 1
            ...
            # print results
            surround_rent_car_url = "http://hotel.elong.com/ajax/detail/getpositionjva/?request.hotelId=" + results[
                "hotel_id"] + "&request.q=租车&request.lng=" + results["longitude"] + "&request.lat=" + results[
                                        "latitude"] + "&request.page_num=1"
            yield Request(surround_rent_car_url, meta={'item': response.meta['item'], "results": results},
                          callback=self.parse_surround_rent_car, dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.ELONG
            error_log_dic['platform_id'] = PlatformEnum.PC
    ...
    def parse_surround_rent_car(self, response):
        try:
            ...
            logstr = {"id": results["hotel_id"], "timestamp": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                      "data": surround_rent_car}
            self.logger_data.info(logstr)
            surround_rent_car_total = surround_rent_car.get('value').get('total')
            ...
            if surround_rent_car_total > 0:
                ...
                for data in trafficItems:
                    if "price" in data.keys():
                        surround_rent_car_list = surround_rent_car_list + str(i) + "##" + data["name"] + "##直线约" + str(
                            data["distance"]) + "米##人均" + str(data["price"]) + "元##" + str(data["address"]) + ";"
                    else:
                        surround_rent_car_list = surround_rent_car_list + str(i) + "##" + data["name"] + "##直线约" + str(
                            data["distance"]) + "米##人均0元##" + str(data["address"]) + ";"
            ...
            t = time.time()
            url = "http://hotel.elong.com/ajax/detail/gethotelreviewcommenttypesandroomtypes?hotelId=" + str(
                results["hotel_id"])  # + '&_='+str(t)
            # url = "http://hotel.elong.com/ajax/detail/gethotelreviewcommenttypesandroomtypes?hotelId=42001245" + '&_='+str(t)

            yield Request(url, meta={'item': response.meta['item'], "results": results}, callback=self.parse_assess,
                          dont_filter=True)
        except:
            error_log_dic = {}
            error_log_dic['channel_id'] = ChannelEnum.ELONG
    ...
    def parse_assess(self, response):
        try:
            ...
            results['guest_impression'] = str(string1)
