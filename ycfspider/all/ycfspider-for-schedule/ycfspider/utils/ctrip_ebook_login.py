...
class CtripEbookLogin():

    def __init__(self):
        self.redis_key =  'login:ctrip_ebook_login'
        self.client = redis.Redis(host=settings.get('REDIS_HOST'),db=0)
    ...
    def login(self,user_name,pwd,channel_number):
        cookie = self.login_util.ctrip_ota_login(user_name,pwd)
        if cookie:
