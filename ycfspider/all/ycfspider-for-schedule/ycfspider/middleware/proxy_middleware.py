...
class ProxyMiddleware(object):
    ...
    def get_ip_from_mongo(self, channel,protocol):
        success_rate = 95
        proxy_list = []
        while True:
            if protocol ==0:
                ...
                proxy = {'success_rate': {'$gt': success_rate},'status': 1,'success': {'$gt': 3}, 'continue_failed': 0,"protocol" : 2}
    ...
    def get_proxy(self, channel=0,protocol=0):
        ...
        if channel == 'QunarPcHotelPriceSpider' or channel =='QunarPcHotelInfoSpider' or channel == 'QunarPcScenicInfoSpider'\
            ...
            # path += '?type=meituan'
            path = 4
        elif channel == 'AlitripPcScenicPriceSpider' or channel=='AlitripPcScenicInfoSpider'or channel == 'AlitripPcScenicInfoMongoSpider':
             # path += '?type=meituan'
            protocol = 2

        # proxy = requests.get(path).content
        # self.proxy = 'http://'+proxy
        self.proxy = 'http://'+self.get_ip_from_mongo(path,protocol)
